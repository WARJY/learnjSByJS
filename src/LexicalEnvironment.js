// 词法环境是ecma262规范规定的规范类型，他包括一个环境记录和一个对外部环境记录的引用，他们互相嵌套并记录了在其关联的词法环境范围内创建的标识符绑定
...
// 词法环境记录共有五种：
// 1.全局环境记录: 最外部的词法环境记录，他的外部引用为空；
// 2.方法环境记录: 当一个方法被声明时创建的环境记录，如function(){}；
// 3.模块环境记录: 当一个模块被声明时创建的环境记录，如module.exports = {}；
// 4.对象环境记录: 当一个对象被声明时创建的环境记录，如a = {}；
// 3.声明环境记录: 当一个声明被声明时创建的环境记录，如let a = 1;
